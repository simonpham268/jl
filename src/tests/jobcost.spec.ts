...
test.describe('[Jobs > Labour] Preserve entered uplift percentage', () => {
  ...
  test('TC_28_RQ2: [Jobs > Labour] Preserve entered uplift percentage of Labour when  uplift is authoritative', async ({ jobService }) => {
  ...
  test('TC_29_RQ2: [Jobs > Overtime] Preserve entered uplift percentage of Overtime when uplift is authoritative', async ({ jobService }) => {
      await systemSetupPage.navigateToSystemSetup();
      await systemSetupPage.clickEdit();
      const roundingConfig: RoundingSettingModel = {
        roundingOption: ROUNDING_OPTION.ROUND_UP,
