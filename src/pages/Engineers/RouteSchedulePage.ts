...
/**
 * RouteSchedulePage - Page Object for Route Schedule page
 * URL: /EngineerTracking/RouteSchedule
 */
export class RouteSchedulePage extends BasePage {
  // ========================
  // Locators - Header
  // ========================
  ...
  constructor(page: Page) {
    ...
    // Header
    this.pageTitle = page.getByRole('heading', { name: /Route Schedule/ });
    ...
    this.routeList = page.locator('ul, ol').first();
    this.loadingIndicator = page.locator('text=Loading');
    this.noResultsMessage = page.locator('text=No route data found');
  }
  ...
  /**
   * Navigate to Route Schedule page
   */
  async navigateToRouteSchedule(): Promise<void> {
    await test.step('Navigate to Route Schedule page', async () => {
      await this.page.goto('/EngineerTracking/RouteSchedule');
  ...
  async assertPageLoaded(): Promise<void> {
    await test.step('Assert Route Schedule page is loaded', async () => {
      await this.page.waitForLoadState('domcontentloaded');
    });
  }
   ...
   */
  async searchRouteSchedule(engineerName: string, date: string): Promise<void> {
    await test.step(`Search route schedule for ${engineerName} on ${date}`, async () => {
