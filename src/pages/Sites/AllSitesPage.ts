 ...
 * Site tabs
 */
export type SiteTab = 'Active' | 'Suspended' | 'All';

/**
 * Table columns that can be sorted
 */
export type SiteSortableColumn = 'Name' | 'Address' | 'Postcode';

/**
 * Results per page options
 */
export type SiteResultsPerPage = 5 | 10 | 20 | 30 | 50;

/**
 * AllSitesPage - Page Object for All Sites list page
 * URL: /Site
 */
export class AllSitesPage extends BasePage {
  // ========================
  ...
  // ========================
  // Locators - Site Detail Sidebar
  // ========================
  readonly siteDetailSidebar: Locator;
  ...
  constructor(page: Page) {
    ...
    // Header/Actions
    this.pageTitle = page.getByRole('heading', { name: 'Sites' });
    this.addSiteButton = page.getByRole('link', { name: 'Add Site' }).first();
    this.printButton = page.getByText('Print');
    this.exportButton = page.getByRole('link', { name: 'Export' });

    // Search/Filter - Note: Sites search has different placeholder
    this.hideFilterButton = page.getByRole('button', { name: /Hide Filter|Show Filter/i });
    this.searchInput = page.getByPlaceholder('Name / Address / Contact Details / Reference / Customer Name / Customer Reference / Customer Account Number / Account Manager / Site Type');
    ...
    this.areaDropdown = page.locator('text=Area').locator('..').locator('[class*="multiselect"], [role="listbox"]');
    ...
    // Site Detail Sidebar
    this.siteDetailSidebar = page.locator('text=Site Summary').locator('..');
    this.viewFullDetailsButton = page.getByRole('button', { name: 'View Full Site Details' });
  }
   ...
   */
  async navigateToAllSites(): Promise<void> {
    await test.step('Navigate to All Sites page', async () => {
  ...
  /**
   * Search sites by text
   * @param query - Search text
   */
  async search(query: string): Promise<void> {
    await test.step(`Search sites: ${query}`, async () => {
  ...
  async filterByArea(area: string): Promise<void> {
    await test.step(`Filter by area: ${area}`, async () => {
      await this.showAdvancedFilters();
   ...
   */
  async searchWithOptions(options: SiteSearchOptions): Promise<void> {
    await test.step('Search with options', async () => {
      ...
      if (options.area) {
        await this.filterByArea(options.area);
        return;
      }
      await this.searchButton.click();
      await this.waitForTableLoad();
  ...
  /**
   * Click on a site row by name
   * @param siteName - Site name to click
   */
  async clickSiteByName(siteName: string): Promise<void> {
    await test.step(`Click site: ${siteName}`, async () => {
      await this.table.getByRole('link', { name: siteName }).click();
    });
  }
  ...
  async getSiteFromRow(index: number): Promise<SiteListItem> {
    ...
    return {
      ...
      address: await cells.nth(2).textContent() || undefined,
      postcode: await cells.nth(3).textContent() || undefined,
      contactName: await cells.nth(4).textContent() || undefined,
      telephone: await cells.nth(5).textContent() || undefined,
      email: await cells.nth(6).textContent() || undefined,
    };
  }
  ...
  /**
   * Check if site exists in the table
   ...
   */
  async setResultsPerPage(count: SiteResultsPerPage): Promise<void> {
  ...
  /**
   * Click Add Site button (navigates to Add Site page)
   */
  async clickAddSite(): Promise<void> {
    await test.step('Click Add Site', async () => {
      await this.addSiteButton.click();
      await this.page.waitForURL(/\/Site\/Create/);
    });
  }
  ...
  /**
   * View full site details (from sidebar)
   */
  async viewFullSiteDetails(): Promise<void> {
    await test.step('View full site details', async () => {
  ...
  /**
   * Check if site detail sidebar is visible
   */
  async isDetailSidebarVisible(): Promise<boolean> {
    return await this.siteDetailSidebar.isVisible();
  }
  ...
  async assertPageLoaded(): Promise<void> {
    await test.step('Assert All Sites page is loaded', async () => {
  ...
  /**
   * Assert search results contain expected site
   * @param siteName - Expected site name in results
   */
